				      ...
				      u32 index)
{
	...
	const u8 *private_updates[2];
	u32 private_update_index[2];
	...
	if (chan) {
		...
		private_update_index[0] = chan->half[0].bcast.index;
		private_update_index[1] = chan->half[1].bcast.index;
		if (private_update_index[0])
			private_updates[0] = gossip_store_get(NULL,
							      rstate->broadcasts->gs,
							      private_update_index[0]);
		else
			private_updates[0] = NULL;
		if (private_update_index[1])
			private_updates[1] = gossip_store_get(NULL,
							      rstate->broadcasts->gs,
							      private_update_index[1]);
		else
			private_updates[1] = NULL;
		/* Pretend it didn't exist, for the moment. */
		tal_free(chan);
	} else
		private_updates[0] = private_updates[1] = NULL;
	...
	if (private_updates[0])
		routing_add_channel_update(rstate, take(private_updates[0]),
					   private_update_index[0]);
	if (private_updates[1])
		routing_add_channel_update(rstate, take(private_updates[1]),
					   private_update_index[1]);
