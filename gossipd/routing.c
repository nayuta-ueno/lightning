					   ...
					   struct routing_state *rstate)
{
	...
	status_trace("Removing node_connection from channels map");
	if (c == NULL)
		return;

	status_trace("Removing node_connection from channels map: FOUND");
	...
	const u8 *announce TAKES)
{
	...
	/* Check if we know the channel already (not matter in what
