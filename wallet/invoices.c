...
static void wallet_stmt2details(const tal_t *ctx,
				sqlite3_stmt *stmt,
...
struct invoice *invoices_all(const tal_t *ctx,
			     struct invoices *invoices)
{
	...
	n = 0;
	ret = tal_arr(ctx, struct invoice, n);
	...
	while (sqlite3_step(stmt) == SQLITE_ROW) {
		tal_resize(&ret, n + 1);
		ret[n].id = sqlite3_column_int64(stmt, 0);
		++n;
	}
	sqlite3_finalize(stmt);
			  ...
			  struct invoice_details *dtl)
{
	...
	wallet_stmt2details(ctx, stmt, dtl);
